...
function extractTransactionsFromPage(transactions, status) {
  ...
  const result = transactions.map(rawTransaction => {
    ...
    const Amount2 = rawTransaction.AmountDouble.toString();
    const Amount3 = Number(Amount2.replace(/\./g,''));
    const newTransaction = {
      date,
      payee_name: rawTransaction.DebitCardFirmName || '',
      notes: rawTransaction.DebitCardFirmName || '',
      amount: Amount3
    };
...
async function fetchTransactionsForAccount(page, startDate, accountId) {
 ...
 const [targetElement1] = await page.$x("//article//span[contains(.,'3875')]");
 await targetElement1.click();
 await hangProcess(4000);
 const [targetElement2] = await page.$x("//article//span[contains(.,'0920')]");
 await targetElement2.click();
 await hangProcess(4000);
 const [targetElement3] = await page.$x("//article//span[contains(.,'3875')]");
 ...
 // const accountNumber = accountId.replace('/', '_').replace(/[^\d-_]/g, '');
  const accountNumber = "leumi3875";
  const response = JSON.parse(responseJson.jsonResp);
  let transactionsToFetch = [];
  let pendingTransactionsToFetch = [];
  let balance = 0;
  if(response.Activity != null && response.Activity.TabNisTransactionItems != null){
    const transactions = response.Activity.TabNisTransactionItems[0].NisTransactionItems;
     transactionsToFetch = extractTransactionsFromPage(transactions, 'completed');
  ...
  if(response.Activity != null && response.Activity.RealTimeTransItems != null){
     ...
     pendingTransactionsToFetch = extractTransactionsFromPage(pendingTransactions, 'pending');
  } else {
    console.log("No Pending Transactions");
  };
  const txns = transactionsToFetch.concat(pendingTransactionsToFetch);
  return {
    accountNumber,
    balance,
    txns
...
class Leumi3875Scraper extends _baseScraperWithBrowser.BaseScraperWithBrowser {
